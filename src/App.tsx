...
import { TaskTimingEditor } from './TaskTimingEditor'
import type { TaskTimingHandle } from './TaskTimingEditor'
import { timingLabel } from './taskTime'
...
import { projectTask, recurrenceLabel, reviewOccurrences } from '../shared/recurrence.mjs'
...
export default function App() {
  ...
  const { tasks: storedTasks, alerts, ready, error, command } = useTasks(setToast)
  ...
  const taskTiming = useRef<TaskTimingHandle>(null)
  ...
  const tasks = storedTasks.map(task => projectTask(task, day))
  const calendarTasks = storedTasks.map(task => projectTask(task, selectedDate))
  ...
  const viewTasks = (view === '日历' ? calendarTasks : tasks).filter(t => view === '今天' ? t.due.startsWith(day) : view === '即将到来' ? !t.done && t.due.slice(0, 10) > day : view === '已完成' ? t.done : view === '日历' ? t.due.startsWith(selectedDate) : categories.includes(view) ? t.category === view : true)
  ...
  async function save(event: FormEvent) {
    ...
    const due = draft.recurrence ? draft.due : reminderTime.current?.commit()
    if (due == null) return
    const timing = taskTiming.current?.commit()
    if (!timing) return
    const old = tasks.find(t => t.id === draft.id)
    const task = { ...draft, ...timing, due, title: draft.title.trim(), reminded: old?.due === due ? old.reminded : false }
  ...
  const nav = [['今天', 'sun'], ['全部任务', 'grid'], ['即将到来', 'calendar'], ['已完成', 'check'], ['时间回顾', 'clock']]
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav>{nav.map(([label, icon]) => <button key={label} className={`nav-item ${view === label ? 'active' : ''}`} onClick={() => { setView(label); setFilter('全部') }}><Icon name={icon}/><span>{label}</span><span className="count">{label === '时间回顾' ? tasks.filter(t => t.completedAt && dateKey(new Date(t.completedAt)) === day).length : label === '今天' ? todayTasks.filter(t => !t.done).length : label === '已完成' ? tasks.filter(t => t.done).length : label === '即将到来' ? pending.filter(t => t.due.slice(0, 10) > day).length : tasks.length}</span></button>)}</nav>
    ...
    <div className="main-shell">
      ...
      <div className={`content-grid ${view === '时间回顾' ? 'review-layout' : ''}`}><main>
        ...
        <div className="data-actions"><span>{isDesktop ? '已保存到本机 · 关闭窗口后在托盘运行' : '数据保存在此浏览器 · 可导出迁移至桌面版'}</span><div><button onClick={() => void exportFile()}>导出任务</button><button onClick={() => importInput.current?.click()}>导入任务</button><button onClick={() => void showStorageInfo()}>数据与备份</button></div><input ref={importInput} className="sr-only" type="file" accept=".json,application/json" title="导入任务文件" aria-label="导入任务文件" onChange={event => { void importFile(event.target.files?.[0]); event.target.value = '' }}/></div><section className="stats"><div className="stat"><span className="stat-icon lavender"><Icon name="calendar"/></span><div><span>今日任务</span><strong>{todayTasks.length}<small>件待办</small></strong></div></div><div className="stat"><span className="stat-icon peach"><Icon name="clock"/></span><div><span>等待完成</span><strong>{todayTasks.length - completed}<small>慢慢来，不着急</small></strong></div></div><div className="stat"><span className="stat-icon mint"><Icon name="check"/></span><div><span>已完成</span><strong>{completed}<small>每一步都是进步</small></strong></div></div></section>
        {view === '时间回顾' ? <TimeReview tasks={reviewOccurrences(storedTasks, reviewDate || day)} day={reviewDate || day} today={day} now={now} onDay={setReviewDate} onEdit={openEditor} onComplete={task => void command({ type: 'complete', id: task.id, due: task.due })}/> : <><button className="review-shortcut" onClick={() => { setReviewDate(view === '日历' ? selectedDate : day); setView('时间回顾') }}><Icon name="clock" size={16}/>查看{view === '日历' ? '当日' : '今日'}时间回顾<Icon name="arrow" size={13}/></button><section className="task-panel"><div className="task-title"><h2>{view === '今天' ? '今日清单' : view === '日历' ? '当日清单' : view}<span>{viewTasks.length}</span></h2><select className="sort" title="任务排序" aria-label="任务排序" value={sort} onChange={e => setSort(e.target.value)}><option>时间排序</option><option>优先级排序</option></select></div><div className="task-toolbar"><div className="tabs">{['全部', '未完成', '已完成'].map(f => <button key={f} onClick={() => setFilter(f)} className={filter === f ? 'selected' : ''}>{f}</button>)}</div><label className="search"><Icon name="search" size={16}/><input placeholder="搜索任务…" title="搜索任务" aria-label="搜索任务" value={query} onChange={e => setQuery(e.target.value)}/></label></div>
        <div className="task-list">{visible.map(t => { const overdue = !t.done && t.due && new Date(t.due).getTime() < now; const paused = Boolean(t.pauses?.length && !t.pauses.at(-1)?.endedAt); return <article className={`task-row ${t.done ? 'is-done' : ''}`} key={t.id}><button className={`checkbox ${t.done ? 'checked' : ''}`} title={`${t.done ? '取消完成' : '完成'}：${t.title}`} aria-label={`${t.done ? '取消完成' : '完成'}：${t.title}`} onClick={async () => { if (await command({ type: 'toggle', id: t.id, due: t.due }) && t.done) setToast('任务已恢复，原起止时间已清空，可重新开始。') }}>{t.done && <Icon name="check" size={14}/>}</button><div className="task-body"><button className="task-name" onClick={() => openEditor(t)}>{t.title}</button>{t.note && <p>{t.note}</p>}<div className="task-meta"><span className={`category-tag tag-${categoryColors[t.category]}`}>{t.category}</span>{t.due && <span className={overdue ? 'overdue' : ''}><Icon name="clock" size={13}/>{t.recurrence ? recurrenceLabel(t) : `${t.due.startsWith(day) ? '今天' : t.due.slice(5, 10).replace('-', '/')} ${t.due.slice(11, 16)}`}{overdue && ' · 已逾期'}</span>}<span className={`priority priority-${t.priority}`}>⚑ {t.priority}优先级</span></div><div className="task-timing">{(t.startedAt || t.completedAt) && <span className={!t.done ? 'running-time' : ''}><Icon name="clock" size={13}/>{timingLabel(t, now, day)}</span>}</div></div><div className="row-actions"><div className="row-edit-actions"><button className="icon-button" title={`编辑：${t.title}`} aria-label={`编辑：${t.title}`} onClick={() => openEditor(t)}><Icon name="edit" size={16}/></button><button className="icon-button delete" title={`删除：${t.title}`} aria-label={`删除：${t.title}`} onClick={async () => { if (await command({ type: 'delete', id: t.id })) setToast('任务已删除') }}><Icon name="trash" size={16}/></button></div>{!t.done && t.startedAt && <button className="start-task" title={`${paused ? '继续计时' : '暂停计时'}：${t.title}`} aria-label={`${paused ? '继续计时' : '暂停计时'}：${t.title}`} onClick={() => void command({ type: paused ? 'resume' : 'pause', id: t.id, due: t.due })}>{paused ? '继续计时' : '暂停计时'}</button>}{!t.done && <button className="start-task" title={`${t.startedAt ? '结束并完成' : '开始'}：${t.title}`} aria-label={`${t.startedAt ? '结束并完成' : '开始'}：${t.title}`} onClick={() => void command({ type: t.startedAt ? 'complete' : 'start', id: t.id, due: t.due })}>{t.startedAt ? '结束并完成' : '开始'}</button>}</div></article> })}{!visible.length && <div className="empty-state"><span>☁</span><h3>{query ? '没有找到匹配的任务' : '这里暂时没有待办'}</h3><p>{query ? '换一个关键词试试吧。' : '留一点空白，或者开始一个新的小计划。'}</p></div>}</div><button className="add-task" onClick={() => openEditor()}><Icon name="plus" size={18}/>添加一个新的小目标<span>让想法落在这里</span></button></section></>}
      ...
      </main><aside className="right-panel"><section className="calendar-card"><div className="calendar-header"><h3>{month.getFullYear()} 年 {month.getMonth() + 1} 月</h3><div><button className="icon-button previous" title="上个月" aria-label="上个月" onClick={() => setMonth(new Date(month.getFullYear(), month.getMonth() - 1, 1))}><Icon name="arrow" size={15}/></button><button className="icon-button" title="下个月" aria-label="下个月" onClick={() => setMonth(new Date(month.getFullYear(), month.getMonth() + 1, 1))}><Icon name="arrow" size={15}/></button></div></div><div className="calendar-grid">{'一二三四五六日'.split('').map(d => <span className="weekday" key={d}>{d}</span>)}{days.map(d => <button key={dateKey(d)} aria-label={dateKey(d)} className={`${d.getMonth() !== month.getMonth() ? 'muted-day' : ''} ${dateKey(d) === day ? 'today' : ''} ${view === '日历' && dateKey(d) === selectedDate ? 'chosen-day' : ''}`} onClick={() => { setSelectedDate(dateKey(d)); setView('日历'); setFilter('全部') }}>{d.getDate()}{storedTasks.some(task => { const t = projectTask(task, dateKey(d)); return !t.done && t.due.startsWith(dateKey(d)) }) && <i/>}</button>)}</div><button className="back-today" onClick={() => { setMonth(new Date()); setView('今天'); setFilter('全部') }}>回到今天</button></section>
    ...
    <dialog ref={storageDialog} className="task-dialog storage-dialog" onClick={event => { if (event.target === event.currentTarget) storageDialog.current?.close() }}>
      <div className="dialog-heading"><h2>数据与备份</h2><button className="icon-button" title="关闭数据与备份" aria-label="关闭数据与备份" onClick={() => storageDialog.current?.close()}><Icon name="close"/></button></div>
    ...
    {toast && <div className="toast" role="status"><Icon name="bell" size={18}/><span>{toast}</span><button title="关闭提示" aria-label="关闭提示" className="icon-button" onClick={() => setToast('')}><Icon name="close" size={15}/></button></div>}
    {showAlerts && <section className="alerts-popover"><div className="section-title"><h3>提醒记录</h3><button className="icon-button" title="关闭提醒" aria-label="关闭提醒" onClick={() => setShowAlerts(false)}><Icon name="close" size={16}/></button></div>{alerts.length ? alerts.map((t, i) => <div className="alert-entry" key={`${t.id}-${i}`}><strong>{t.title}</strong><small>{t.due.replace('T', ' ')} 已到期</small></div>) : <p>暂无提醒。为任务设置时间，到期后会在这里提醒你。</p>}</section>}
    <dialog ref={dialog} className="task-dialog" onClick={e => { if (e.target === e.currentTarget) dialog.current?.close() }}><form onSubmit={save}><div className="dialog-heading"><div><span className="eyebrow">ONE LITTLE STEP</span><h2>{editing ? '编辑任务' : '一个新的小目标'}</h2></div><button type="button" className="icon-button" title="关闭编辑" aria-label="关闭编辑" onClick={() => dialog.current?.close()}><Icon name="close"/></button></div><label>准备做点什么？<input autoFocus required maxLength={120} placeholder="写下你想完成的事…" value={draft.title} onChange={e => setDraft({ ...draft, title: e.target.value })}/></label><label>备注<textarea rows={3} maxLength={1000} placeholder="补充一点细节（选填）" value={draft.note} onChange={e => setDraft({ ...draft, note: e.target.value })}/></label><div className="form-columns"><label>分类<select value={draft.category} onChange={e => setDraft({ ...draft, category: e.target.value })}>{categories.map(c => <option key={c}>{c}</option>)}</select></label><label>优先级<select value={draft.priority} onChange={e => setDraft({ ...draft, priority: e.target.value })}>{['高', '中', '低'].map(p => <option key={p}>{p}</option>)}</select></label></div><RecommendationTaskFields task={draft} onChange={setDraft}/><RecurrenceFields task={draft} onChange={setDraft}/>{!draft.recurrence && storedTasks.find(task => task.id === draft.id)?.recurrence && <p className="form-help">改为不重复后，仅保留当前这次任务和用时，此前周期记录将移除。需要保留完整记录时，请先取消并导出任务。</p>}{!draft.recurrence && <ReminderTime key={editorSession} ref={reminderTime} value={draft.due} onChange={due => setDraft(previous => ({ ...previous, due }))}/>}<p className="form-help">{draft.recurrence ? '周期提醒使用本机时间；程序运行时自动提醒。' : isDesktop ? '时间可留空。程序在托盘运行时也会提醒。' : '时间可留空。页面打开时，到期会自动提醒。'}</p><TaskTimingEditor key={`timing-${editorSession}`} ref={taskTiming} task={draft}/><div className="dialog-footer"><button className="secondary-button" type="button" onClick={() => dialog.current?.close()}>取消</button><button className="primary-button" type="submit">{editing ? '保存修改' : '添加任务'}<Icon name="check" size={17}/></button></div></form></dialog>
  </div>
}
