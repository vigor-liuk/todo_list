...
export type Pause = { startedAt: string; endedAt?: string }
export type OccurrenceState = { done: boolean; reminded: boolean; startedAt?: string; completedAt?: string; pauses?: Pause[] }
...
export type Task = { id: string; title: string; note: string; category: string; priority: string; due: string; done: boolean; reminded: boolean; startedAt?: string; completedAt?: string; pauses?: Pause[]; recommendation?: RecommendationRecord; recurrence?: Recurrence }
export type Command = { type: 'upsert'; task: Task } | { type: 'delete' | 'toggle' | 'start' | 'pause' | 'resume' | 'complete'; id: string; due?: string } | { type: 'import'; text: string }
